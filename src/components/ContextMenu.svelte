...
<script>
    ...
    let showMenu = false;
    // showGlobalMenu is state of global context-menu visibility
    let showGlobalMenu = false;
    // to display some text
    let content;

    function rightClickContextMenu(e) {
        // check if click happen inside containing element
        showMenu = true
    ...
    let menuItemsGlobalContextMenu = {
        'name': 'rightClickGlobalContextMenu',
        'onClick': function () {
        },
        'displayText': "Right Click Global Context Menu",
        'class': 'fa-solid fa-mouse'
    };

    function rightClickGlobalContextMenu(e) {
        // check if click happen inside containing element
        showMenu = false
        showGlobalMenu = true
        browser = {
        ...
        <div class="navbar" id="navbar">
            <ul>
                {#each menuItems as item}
                    ...
                    {:else}
                        <li>
                            <button on:click={item.onClick}><i class={item.class}></i>{item.displayText}</button>
                        </li>
                    {/if}
                {/each}
            </ul>
